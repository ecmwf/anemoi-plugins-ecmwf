    ...
    def test_mir_vs_ectrans_comparison(self):
        ...
        for name in ["ctrans4py", "ectrans4py"]:
            if name in BACKENDS:
                ...
                available, _ = backend_class.available()
                if available:
                    ectrans_backend_class = backend_class
                    break

        if ectrans_backend_class is None:
            pytest.skip("No ectrans backend available for comparison")

        mir_backend_class = BACKENDS["mir"]
        trunc = 21
        kloen = grid_to_pl("O22")

        # Create backends
        mir_backend = mir_backend_class(kloen, trunc, grid="O22")
        ectrans_backend = ectrans_backend_class(kloen, trunc)

        # Create a spectral field with some structure
        nspec = nspec_from_trunc(trunc)
        np.random.seed(42)
        spectral = np.random.randn(nspec) * 10.0
        spectral[0] = 300.0  # Set mean

        # Transform with both backends
